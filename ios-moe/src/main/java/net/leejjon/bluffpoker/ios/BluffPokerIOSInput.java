...
public class BluffPokerIOSInput implements Input {
    ...
    Pool<BluffPokerIOSInput.TouchEvent> touchEventPool = new Pool<BluffPokerIOSInput.TouchEvent>() {
        @Override
        protected BluffPokerIOSInput.TouchEvent newObject () {
            return new BluffPokerIOSInput.TouchEvent();
        }
    };
    Array<BluffPokerIOSInput.TouchEvent> touchEvents = new Array<>();
    BluffPokerIOSInput.TouchEvent currentEvent = null;
    ...
    public BluffPokerIOSInput(BluffPokerIOSApplication app) {
        this.app = app;
        this.config = app.config;
        this.keyboardCloseOnReturn = app.config.keyboardCloseOnReturn;
    }

    void setupPeripherals (BluffPokerGame game) {
        motionManager = CMMotionManager.alloc().init();
        setupAccelerometer(game);
        setupCompass();
     ...
     * @param game
     */
    private void setupAccelerometer (BluffPokerGame game) {
    ...
    private final UITextFieldDelegate textDelegate = new UITextFieldDelegate() {
        ...
        public boolean textFieldShouldChangeCharactersInRangeReplacementString (UITextField textField, NSRange range, String string) {
            for (int i = 0; i < range.length(); i++) {
                app.input.getInputProcessor().keyTyped((char)8);
            }
            ...
            for (int i = 0; i < chars.length; i++) {
                app.input.getInputProcessor().keyTyped(chars[i]);
            }
        ...
        public boolean textFieldShouldReturn (UITextField textField) {
            ...
            app.input.getInputProcessor().keyDown(Keys.ENTER);
            app.input.getInputProcessor().keyTyped((char)13);
            Gdx.graphics.requestRendering();
            return false;
        }
    };
    ...
    void processEvents () {
        synchronized (touchEvents) {
            justTouched = false;
            for (BluffPokerIOSInput.TouchEvent event : touchEvents) {
                currentEvent = event;
    ...
    private void toTouchEvents (NSSet<? extends UITouch> touches) {
        for (UITouch touch : touches.allObjects()) {
            ...
            synchronized (touchEvents) {
                ...
                BluffPokerIOSInput.TouchEvent event = touchEventPool.obtain();
                event.x = locX;
                event.y = locY;
                event.phase = phase;
