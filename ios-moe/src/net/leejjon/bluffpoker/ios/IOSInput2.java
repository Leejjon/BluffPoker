...
public class IOSInput2 implements Input {
    ...
    Pool<IOSInput2.TouchEvent> touchEventPool = new Pool<IOSInput2.TouchEvent>() {
        @Override
        protected IOSInput2.TouchEvent newObject () {
            return new IOSInput2.TouchEvent();
        }
    };
    Array<IOSInput2.TouchEvent> touchEvents = new Array<>();
    IOSInput2.TouchEvent currentEvent = null;
    ...
    public IOSInput2 (IOSApplication2 app) {
        this.app = app;
...
//            motionManager.startAccelerometerUpdatesToQueueWithHandler(NSOperationQueue.alloc().init(), handler);
    ...
    void processEvents () {
        synchronized (touchEvents) {
            justTouched = false;
            for (IOSInput2.TouchEvent event : touchEvents) {
                currentEvent = event;
    ...
    private void toTouchEvents (NSSet<? extends UITouch> touches) {
        for (UITouch touch : touches.allObjects()) {
            ...
            synchronized (touchEvents) {
                ...
                IOSInput2.TouchEvent event = touchEventPool.obtain();
                event.x = locX;
                event.y = locY;
                event.phase = phase;
